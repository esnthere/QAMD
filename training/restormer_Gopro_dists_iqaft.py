...
class Mydataset(Dataset):
    ...
    def __getitem__(self, index):
        return torch.from_numpy(self.imgs[index]),torch.from_numpy(self.targets[index]),torch.from_numpy(self.imgs_large[index]),torch.from_numpy(self.targets_large[index])
...
def main():
    ...
    all_data = sio.loadmat('E:\Database\Deblur_datasets\GOPRO_Large\Gopro_Area_280_140_70.mat')
    X = all_data['X']
    Y = all_data['Y']
    ...
    all_data = np.load('E:\Database\Deblur_datasets\GOPRO_Large\Gopro_Area_720_360_180.npz')
    X_large = all_data['X']
    Y_large = all_data['Y']

    Xtest_large = all_data['Xtest']
    Ytest_large = all_data['Ytest']
    ...
    pretrained_dict = torch.load('restormer_gopro_iqaft_bl.pt')
    ...
    for epoch in range(epochs):
        ...
        if epoch %2==0 and epoch>1:
            ...
            ct+=1
            if min_plsp > loss:
                save_nm = 'restormer_gopro_iqa.pt'
                min_plsp = loss
